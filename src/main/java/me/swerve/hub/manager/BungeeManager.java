...
public class BungeeManager implements PluginMessageListener {
    ...
    public BungeeManager() {
        instance = this;
        Bukkit.getScheduler().runTaskTimerAsynchronously(RiseHub.getInstance(), () -> getOnline("ALL"), 0L, 20L);
    }
    ...
    public void ping(String fullIp) {
        Bukkit.getScheduler().runTaskAsynchronously(RiseHub.getInstance(), () -> {
            try {
    ...
    public int getOnline(String server) {
        try {
            ...
            out.writeUTF(server);
            Bukkit.getServer().sendPluginMessage(RiseHub.getInstance(), "BungeeCord", out.toByteArray());
